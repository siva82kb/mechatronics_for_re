\documentclass{standalone}
\usepackage{tikz}
\usepackage{pgfplots}
\pgfplotsset{compat=newest}
\usepackage{amsmath}
\usepackage[american]{circuitikz}
\usepackage{cmbright}

\definecolor{myred}{RGB}{170,0,0}
\definecolor{myblue}{RGB}{0,0,220}
\definecolor{mygreen}{RGB}{0,150,0}
\definecolor{myorange}{RGB}{255,127,0}
\definecolor{mybrown}{RGB}{150,75,0}

\begin{document}
\begin{tikzpicture}
    \begin{scope}
        % Title
        \node[anchor=center, color=myred] at (-2, 3.75) {\large $npn$ BJT to switch on/off an LED};
        % npn BJT sumbol
        \draw (0, -1.25) node[npn, scale=2.0] (Q) {};
        \draw (Q.base)
            to[R, R={$R_B$}, i<={$i_B$}] ++(-2.0, 0)
            to[short, -o, invert] ++(-0.5, 0);
        \node[anchor=center, align=center] at ($(Q.base) + (-2.5, 0.25)$) {$V_{in}$};
        \draw ($(Q.collector) + (0, -0.35)$)
            to[R, R={$R_{led}$}] ++(0, 1.9)
            to[led, -o, l_=$D$, invert] ++(0, 1.3);
        \node[anchor=center, align=center] at ($(Q.collector) + (0.35, 3.0)$) {$V_{CC}$};
        \node[ground] at (Q.emitter) {};
    \end{scope}
\end{tikzpicture}
\end{document}